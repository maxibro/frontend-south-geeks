...
export const get = (args: IGetActionData) => {
  return (dispatch: Dispatch) => {
    ...
    const promise = new Promise((resolve, reject) => {
      const { data } = args;
      const url: RequestInfo = `${BACKEND_PROTO}://${BACKEND_HOSTNAME}:${BACKEND_PORT}/order/${data.id}`;
      axios.get(url).then(
